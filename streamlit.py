...
DB_CONFIG = dict(host="localhost", user="root", password="", database="project2_ida")
...
query_map = {
    "1. Retrieve all distinct country names": 
                """SELECT DISTINCT `Country Name` FROM project2_ida.Country_Debt ORDER BY `Country Name`;""",
    "2. Count the total number of countries available":
                """SELECT COUNT(DISTINCT `Country Name`) AS Total_Countries FROM project2_ida.Country_Debt;""",
    "3. Find the total number of indicators present": 
                """SELECT COUNT(DISTINCT `Series Code`) AS Total_Indicators FROM project2_ida.Country_Debt;""",
    "4. Display the first 10 records of the dataset": 
                """SELECT * FROM project2_ida.Country_Debt LIMIT 10;""",
    "5. Calculate the total global debt": 
                """SELECT SUM(`Value`) AS Total_Global_Debt FROM project2_ida.Country_Debt;""",
    "6. List all unique indicator names": 
                """SELECT DISTINCT `Indicator Name` FROM project2_ida.indicator_metadata ORDER BY `Indicator Name`;""",
    "7. Find the number of records for each country": 
                """SELECT `Country Name`, COUNT(*) AS Record_Count FROM project2_ida.Country_Debt 
                GROUP BY `Country Name` 
                ORDER BY Record_Count DESC;""",
    "8. Display all records where debt is greater than 1 billion USD": 
                """SELECT * FROM project2_ida.Country_Debt WHERE `Value` > 1000000000;""",
    "9. Find the minimum, maximum, and average debt values": 
                """SELECT MIN(`Value`) AS Min_Debt, MAX(`Value`) AS Max_Debt, AVG(`Value`) AS Avg_Debt FROM project2_ida.Country_Debt;""",
    "10. Count total number of records in the dataset": 
                """SELECT COUNT(*) AS Total_Records FROM project2_ida.Country_Debt;""",
    "11. Find the country with the lowest total debt": 
                """SELECT `Country Name`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt 
                GROUP BY `Country Name` ORDER BY Total_Debt ASC LIMIT 1;""",
    "12. Calculate total debt for each country and indicator combination": 
                """SELECT `Country Name`, `Series Code`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt
                GROUP BY `Country Name`, `Series Code` ORDER BY Total_Debt DESC;""",
    "13. Count how many indicators each country has": 
                """SELECT `Country Name`, COUNT(DISTINCT `Series Code`) AS Indicator_Count FROM project2_ida.Country_Debt 
                GROUP BY `Country Name` ORDER BY Indicator_Count DESC;""",
    "14. Display countries whose total debt is above the global average": 
                """SELECT `Country Name`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt 
                GROUP BY `Country Name` HAVING SUM(`Value`) > (SELECT AVG(total_debt) FROM (SELECT SUM(`Value`) AS total_debt FROM project2_ida.Country_Debt 
    ...
    "15. Rank countries based on total debt (highest to lowest)": 
                ...
                ORDER BY Total_Debt DESC;""",
    "16. Top 5 indicators contributing most to global debt": 
                """SELECT `Series Name`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt 
                GROUP BY `Series Name` 
                ORDER BY Total_Debt DESC LIMIT 5;""",
    "17. Percentage contribution of each country to total global debt": 
                """SELECT `Country Name`, SUM(`Value`) AS Total_Debt, ROUND(SUM(`Value`)/global.Global_Total*100, 2) AS Percent_Global FROM project2_ida.Country_Debt, (SELECT SUM(`Value`) AS Global_Total FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL) AS global 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name` 
                ORDER BY Percent_Global DESC;""",
    "18. Top 3 countries for each indicator based on debt": 
                """SELECT `Series Name`, `Country Name`, Total_Debt FROM (SELECT `Series Name`, `Country Name`, SUM(`Value`) AS Total_Debt, ROW_NUMBER() OVER (PARTITION BY `Series Name` ORDER BY SUM(`Value`) DESC) AS rn FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL GROUP BY `Series Name`, `Country Name`) AS ranked 
                WHERE rn <= 3 ORDER BY `Series Name`, Total_Debt DESC;""",
    "19. Difference between maximum and minimum debt for each country": 
                """SELECT `Country Name`, MAX(`Value`) - MIN(`Value`) AS Debt_Difference FROM project2_ida.Country_Debt 
                GROUP BY `Country Name` 
                ORDER BY Debt_Difference DESC;""",
    "20. Create a view for the top 10 countries with highest debt": 
                """CREATE OR REPLACE VIEW project2_ida.top_10_countries_debt AS SELECT `Country Name`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name` 
                ORDER BY Total_Debt DESC LIMIT 10;""",
    "21. Categorize countries into High/Medium/Low Debt": 
                """SELECT `Country Name`, SUM(`Value`) AS Total_Debt, CASE WHEN SUM(`Value`) > 1000000000000 THEN 'High Debt' WHEN SUM(`Value`) > 100000000000 THEN 'Medium Debt' ELSE 'Low Debt' END AS Debt_Category FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name` 
                ORDER BY Total_Debt DESC;""",
    "22. Calculate cumulative debt per country with window functions": 
                """SELECT `Country Name`, `Series Name`, Total_Debt, SUM(Total_Debt) OVER (PARTITION BY `Country Name` ORDER BY Total_Debt DESC) AS Cumulative_Debt FROM (SELECT `Country Name`, `Series Name`, SUM(`Value`) AS Total_Debt FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name`, `Series Name`) AS country_indicator 
                ORDER BY `Country Name`, Cumulative_Debt DESC;""",
    "23. Indicators where average debt is higher than overall average debt": 
                """SELECT `Series Name`, AVG(`Value`) AS Avg_Debt FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Series Name` HAVING AVG(`Value`) > (SELECT AVG(`Value`) FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL) 
                ORDER BY Avg_Debt DESC;""",
    "24. Countries contributing more than 5% of global debt": 
                """SELECT `Country Name`, SUM(`Value`) AS Country_Debt, ROUND((SUM(`Value`) / (SELECT SUM(`Value`) FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL)) * 100, 2) AS Debt_Percentage FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name` HAVING Debt_Percentage > 5 ORDER BY Debt_Percentage DESC;""",
    "25. Most dominant indicator for each country": 
                """SELECT `Country Name`, `Series Name`, Total_Debt FROM (SELECT `Country Name`, `Series Name`, SUM(`Value`) AS Total_Debt, ROW_NUMBER() OVER (PARTITION BY `Country Name` ORDER BY SUM(`Value`) DESC) AS rn FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL
    ...
    "27. Display the top 10 countries with the highest total debt": 
                ...
                LIMIT 10;""",
    "28. Find the average debt per country": 
                """SELECT `Country Name`, AVG(`Value`) AS Avg_Debt FROM project2_ida.Country_Debt 
                WHERE `Value` IS NOT NULL 
                GROUP BY `Country Name` 
                ORDER BY Avg_Debt DESC;""",
    "29. Calculate total debt for each indicator": 
...
if selected_query == "1. Retrieve all distinct country names":
    ...
    if df.empty:
        st.success("View `project2_ida.top_10_countries_debt` created or already exists.")
    else:
        st.dataframe(df)
